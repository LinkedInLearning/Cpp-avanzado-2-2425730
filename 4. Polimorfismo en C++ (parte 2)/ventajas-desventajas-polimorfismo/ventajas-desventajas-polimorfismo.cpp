...
int main(){
    ...
    // Empleado *supervisor = new Supervisor("Dwight", "Schrute", 18);
    // Empleado *director = new Director("Michael", "Scott", 1, 90500.30);

    // supervisor->mostrarInformacion();
    // director->mostrarInformacion();

    vector<Empleado*> cargosDeGestion;

    cargosDeGestion.push_back(new Supervisor("Dwight", "Schrute", 18));
    cargosDeGestion.push_back(new Director("Michael", "Scott", 1, 90500.30));
    cargosDeGestion.push_back(new Supervisor("Jim", "Halpert", 3));
    cargosDeGestion.push_back(new Director("Pam", "Beesly", 30, 10300.46));

    for(int i=0; i<cargosDeGestion.size(); i++){
        cargosDeGestion.at(i)->mostrarInformacion();
    }
